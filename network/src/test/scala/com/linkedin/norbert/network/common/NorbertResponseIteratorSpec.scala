...
class NorbertResponseIteratorSpec extends SpecificationWithJUnit with Mockito with SampleMessage {
  ...
  trait NorbertResponseIteratorSetup extends Scope {
    ...
    class DummyInputSerializer[X, Y] extends InputSerializer[X, Y] {
      ...
      def requestFromBytes(bytes: Array[Byte]): X = {
        return null.asInstanceOf[X]
      }

      def responseFromBytes(bytes: Array[Byte]): Y = {
        return null.asInstanceOf[Y]
      }
    }
    ...
    class DummyOutputSerializer[X, Y] extends OutputSerializer[X, Y] {
      ...
      def requestToBytes(request: X): Array[Byte] = {
        return Array.empty[Byte]
      }

      def responseToBytes(response: Y): Array[Byte] = {
        return Array.empty[Byte]
      }
    }
  ...
  "NorbertResponseIterator" should {
    ...
    "Handle exceptions at the end of the stream" in {
      ...
      partialIterator.hasNext must_== true
      partialIterator.next must_== 0

      partialIterator.hasNext must_== true
      partialIterator.next must_== 1

      partialIterator.hasNext must_== false
    }
    ...
    //duplicates not ok
    "if not timeout occurs behavior is unchanged in case initial timeout occurs but not the latter timeout" in new NorbertResponseIteratorSetup {
      ...
      val insertQueueLate1 = new Thread(new Runnable {
        def run() {
          Thread.sleep(100);
          queue += Right(Tuple3(node1, Set.empty[Int] + 1, 1000))
        }
      })
    ...
    //duplicates not ok
    "if not timeout occurs behavior is unchanged in case timeout occurs" in new NorbertResponseIteratorSetup {
      def calculateNodeFunctor(setPIds: Set[Int], setNodes: Set[Node], requestMsg: Int) = Map.empty[Node, Set[Int]]
    ...
    "if timeout occurs behavior is unchanged as compared to before" in new NorbertResponseIteratorSetup {
      def calculateNodeFunctor(setPIds: Set[Int], setNodes: Set[Node], requestMsg: Int) = Map.empty[Node, Set[Int]]
      ...
      val insertQueueTimely = new Thread(new Runnable {
        def run() {
          Thread.sleep(15);
          queue += Right(Tuple3(Node(10, "endpoint", true), Set.empty[Int] + 300, 3000))
        }
      })
      ...
      val insertQueueLate = new Thread(new Runnable {
        def run() {
          Thread.sleep(50);
          queue += Right(Tuple3(Node(10, "endpoint", true), Set.empty[Int] + 400, 4000))
        }
      })
      ...
      timeoutExceptionFlag must_== true
    }

    "if timeout occurs we try to find the next node multiple times" in new NorbertResponseIteratorSetup {
      ...
      def callback(pRequest: PartitionedRequest[Int, Int, Int]) = {
        if (invocationCount == 1) {
          val insertQueueLate = new Thread(new Runnable {
            def run() {
              Thread.sleep(5000);
              queue += Right(Tuple3(Node(6, "endpoint", true), Set.empty[Int] + 5 + 6, 6000))
            }
          })
          insertQueueLate.start()
          val insertQueueLate1 = new Thread(new Runnable {
            def run() {
              Thread.sleep(10);
              queue += Right(Tuple3(Node(4, "endpoint", true), Set.empty[Int] + 1 + 2, 4000))
            }
          })
          insertQueueLate1.start()
          val insertQueueLate2 = new Thread(new Runnable {
            def run() {
              Thread.sleep(15);
              queue += Right(Tuple3(Node(5, "endpoint", true), Set.empty[Int] + 3 + 4, 5000))
            }
          })
          ...
          val insertQueueLate = new Thread(new Runnable {
            def run() {
              Thread.sleep(20);
              queue += Right(Tuple3(Node(7, "endpoint", true), Set.empty[Int] + 5 + 6, 9000))
            }
          })
          insertQueueLate.start()
        }
    ...
    "in case of duplicates we should return duplicate values and stop as soon as we have complete partitions" in new NorbertResponseIteratorSetup {
      def calculateNodeFunctor(setPIds: Set[Int], setNodes: Set[Node], requestMsg: Int) = Map.empty[Node, Set[Int]]
    ...
    "in case of duplicate outstanding requests the first one to complete wins" in new NorbertResponseIteratorSetup {
      def calculateNodeFunctor(setPIds: Set[Int], setNodes: Set[Node], requestMsg: Int): Map[Node, Set[Int]] = {
        val failedNodes = Set.empty[Node] + Node(1, "node1", true) + Node(2, "node2", true)
        setNodes.size mustEqual 2
        setNodes mustEqual failedNodes
      ...
      def callback(pRequest: PartitionedRequest[Int, Int, Int]) = {
        ...
        val insertQueueLate1 = new Thread(new Runnable {
          def run() {
            Thread.sleep(100);
            queue += Right(Tuple3(Node(4, "endpoint", true), Set.empty[Int] + 2, 4000))
          }
        })
        insertQueueLate1.start()
      }
      ...
      val insertQueueLate1 = new Thread(new Runnable {
        def run() {
          Thread.sleep(100);
          queue += Right(Tuple3(Node(1, "endpoint", true), Set.empty[Int] + 1, 1000))
        }
      })
      insertQueueLate1.start()
      val insertQueueLate2 = new Thread(new Runnable {
        def run() {
          Thread.sleep(90);
          queue += Right(Tuple3(Node(2, "endpoint", true), Set.empty[Int] + 2, 2000))
        }
      })
      insertQueueLate2.start()
